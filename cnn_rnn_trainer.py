...
parser.add_argument('--cnn_save_path', type=str, default='../cs231n_data/saved_rnn_models/best_cnn_model.cris')
parser.add_argument('--rnn_save_path', type=str, default='../cs231n_data/saved_rnn_models/best_rnn_model.cris')
parser.add_argument('--save_loss_path', default='../cs231n_data/saved_rnn_models/loss.txt')
...
parser.add_argument('--load_initial', action='store_true')
...
def main(args):
    ...
    if os.path.isfile(args.rnn_save_path) and not args.load_initial:
    ...
    if args.load_initial:
        encoder.load_state_dict(torch.load(args.cnn_save_path))
        decoder.load_state_dict(torch.load(args.rnn_save_path))
    ...
    best_f2 = -np.inf
    if args.load_initial:
        best_f2 = check_f2(encoder, decoder, val_loader, dtype, is_cnn_training = False)
        print('Starting with Val f2 = %f' % (best_f2))
